<template>
        ...
        <div class="rounded-t mb-0 px-4 py-3 border-0">
          <div class="flex flex-wrap items-center">
            <div class="relative w-full px-4 max-w-full flex-grow flex-1">
              <h3 class="font-semibold text-base text-blueGray-700">Enfant Enregistrés</h3>
            </div>
          ...
          <table class="items-center bg-transparent w-full border-collapse ">
            ...
            <tbody>
              <tr v-for="insc in inscription " :key="insc.Id_Incription">
                ...
                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                  <NuxtLink :to="`/inscription/${insc.Id_Incription}`"  class="bg-green-500 text-white active:bg-indigo-600 text-xs font-bold uppercase px-3 py-1 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button">Inscrire</NuxtLink>
                </td>
    ...
    const router = useRoute()
     const {data:inscriptions} = await $fetch(`/api/listinscris/${router.params.id}`)
